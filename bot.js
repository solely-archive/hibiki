...
Hibiki.dispatcher.addInhibitor((msg) => {
  ...
  const message = blacklist.replace(/(<user>)/, msg.author.username).replace(/(<bot>)/, Hibiki.user.username);
...
Hibiki.once('ready', () => Currency.leaderboard())
  .on('message', async (msg) => {
    ...
    if (/(discord(\.gg\/|app\.com\/invite\/|\.me\/|\.io\/))/gi.test(msg.content)) {
      if (
        msg.author.bot ||
        msg.member.permissions.has('MANAGE_SERVER') ||
        msg.member.roles.has(msg.guild.settings.get('antiInviteRole'))
      )
        return;
    ...
    if (!gainedXPRecently.includes(msg.author.id)) {
      ...
      Experience.addExperience(msg.author.id, xpEarned)
        .then(async () => {
