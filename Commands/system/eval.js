...
module.exports = class Eval extends Command {
  ...
  async run(msg, args) {
    try {
      ...
      let data = `${responseTypeOf.replace(
        new RegExp(
          `
                ${this.client.token}|
                ${TOKEN}|
      ...
      if (data.length > 1024) {
        ...
        return msg.say(
          `The eval response is more than 1024 characters, so I uploaded the contents here.\n${resp.body.html_url}`
        );
      }
      ...
      const embed = new MessageEmbed()
        ...
        .setFooter(`${this.client.version}`)
        .addField('Error', `\`\`\`js\n${err.message}\`\`\``, true);
      await msg.embed(embed).catch((err) => this.client.logger.error(err.stack));
    }
  }
