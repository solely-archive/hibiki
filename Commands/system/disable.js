...
module.exports = class Disable extends Command {
  ...
  run(msg, { cmdOrGrp }) {
    if (!cmdOrGrp.isEnabledIn(msg.guild, true)) {
      return msg.say(`❎ | The \`${cmdOrGrp.name}\` ${cmdOrGrp.group ? 'command' : 'group'} is already disabled.`);
    }
    if (cmdOrGrp.guarded) {
      return msg.say(`❎ | You cannot disable the \`${cmdOrGrp.name}\` ${cmdOrGrp.group ? 'command' : 'group'}.`);
    }
