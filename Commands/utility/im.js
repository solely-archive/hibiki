...
module.exports = class ImRoleCommand extends Command {
  ...
  async run(msg, { role }) {
    ...
    if (!settings || !settings.assignableRoles || !settings.assignableRoles.roles)
      return msg.reply('There are no self-assignable roles set up.');
    let assignableRoles = settings.assignableRoles.roles;
    if (!assignableRoles.includes(role.id))
      return msg.say(`The role ${role.name} is not available as a self-assignable role.`);
