    ...
    super(client, {
      ...
      args: [
        ...
        },
        {
          key: 'message',
          prompt:
            'What should be sent to the channel? Use <user> (username), <server> (server name), and <mention> (user mention) as placeholders.\n',
          type: 'string',
          validate: (message) => {
  ...
  run(msg, args) {
    ...
    if (type === 'welcomeMsg') {
      ...
      const embed = new MessageEmbed()
        ...
        .setColor(0x00ff00)
        .setDescription(
          `The welcome message will now look like this:\n**${message
      ...
      const embed = new MessageEmbed()
        ...
        .setColor(0x00ff00)
        .setDescription(
          `The farewell message will now look like this:\n**${message
            .replace(/(<user>)/gi, msg.author.username)
            .replace(/(<server>)/gi, msg.guild.name)
            .replace(/(<mention>)/gi, msg.author.toString())}**`
        );
