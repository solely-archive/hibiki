...
module.exports = class RemoveAssignableRoleCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      guildOnly: true,
      examples: ['remove-assignable-role frens'],
      args: [
        {
          key: 'role',
  ...
  async run(msg, { role }) {
    const settings =
      (await Guild.findOne({ where: { guildID: msg.guild.id } })) || (await Guild.create({ guildID: msg.guild.id }));
    if (!settings.assignableRoles.roles) return msg.reply('there are no self-assignable roles on the list.');
    if (!settings.assignableRoles.roles.includes(role.id))
      return msg.reply(`${role.name} is not in the list of self-assignable roles.`);
