...
module.exports = class BlackjackCommand extends Command {
  ...
  run(msg, { bet }) {
    ...
    return msg
      .say(`New game of blackjack started with ${msg.member.displayName} with a bet of ${Currency.convert(bet)}!`)
      .then(async () => {
        const balance = await Currency.getBalance(msg.author.id);
        ...
        playerHands.forEach((hand, i) => {
          ...
          embed.fields.push({
            ...
            value: stripIndents`
						...
						Result: ${result.replace(/(^\w|\s\w)/g, (ma) => ma.toUpperCase())}${
        ...
        embed.description = `You ${
          winnings === 0 ? 'broke even' : `${winnings > 0 ? 'won' : 'lost'} ${Currency.convert(Math.abs(winnings))}`
        }`;
    ...
    return new Promise(async (resolve) => {
      ...
      while (currentHand) {
        ...
        await msg.embed({
          ...
          title: `Blackjack | ${msg.member.displayName}`,
          description:
            !canDoubleDown && !canSplit
              ? 'Type `hit` to draw another card or `stand` to pass.'
              : `Type \`hit\` to draw another card, ${canDoubleDown ? '`double down` to double down, ' : ''}${
        ...
        const responses = await msg.channel.awaitMessages(
          (
            msg2 // eslint-disable-line no-await-in-loop
          ) =>
